                    ...
                    <div class="discover__select flex space-x-5">

                        {
                            filterGenre.map(genre => {
                                return (
                                    <div className={`discover__select-item border py-1 px-4 rounded-3xl cursor-pointer ${genre.name === onSelect ? 'bg-gray-300' : 'bg-gray-200'}`} onClick={() => setOnSelect(genre.name)}>
                                      style={ !onSelect ? { backgroundColor: 'red' } : null }
                                        onclick={() => {
                                            // setFiltesrGenre(genre.name)

                                            setGenre(genre.name)
                                            setOnSelect(genre.name)
